...
interface ProjectRoadmapProps {
  briefingStatus: "empty" | "draft" | "awaiting_response" | "sent" | "approved";  // ✅ Adicionado
  projectStatus: "active" | "paused" | "done" | "archived";
}


export function ProjectRoadmap({ briefingStatus, projectStatus }: ProjectRoadmapProps) {
...
const currentStepIndex = useMemo(() => {
  ...
  return (
    <div className="mt-6 mb-12 w-full rounded-3xl border border-zinc-800/60 bg-zinc-950/40 backdrop-blur-sm overflow-hidden relative group">
      {/* HEADER DO CARD */}
      ...
      <div className="p-6 overflow-x-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-800/50 pb-8">
        <div className="min-w-[700px] flex items-center justify-between relative px-4">
          ...
          {steps.map((step, i) => {
            ...
            const isCurrent = i === currentStepIndex && projectStatus !== "done";

            return (
              <div key={i} className="flex flex-col items-center gap-4 relative group/step cursor-default w-24">
                {/* Ícone da Etapa */}
                <motion.div 
