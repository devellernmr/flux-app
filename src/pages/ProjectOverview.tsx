...
export function ProjectOverview() {
  ...
  const loadTemplate = (key: keyof typeof BRIEFING_TEMPLATES) => {
    setBlocks([...BRIEFING_TEMPLATES[key].blocks]);
    setIsEditing(true);
  };
  ...
  const resetBriefing = async () => {
    if (
      !confirm(
        "Tem certeza? Isso apaga as perguntas atuais e volta para a seleção."
      )
    )
      return;
      ...
      <header className="h-14 border-b border-slate-800 flex items-center px-4 justify-between bg-[#020617] shrink-0 z-10">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            className="h-8 text-xs border-slate-700 bg-transparent hover:bg-slate-800"
            onClick={copyLink}
          >
                        ...
                        {briefingStatus === "empty" && !isEditing && (
                          <div className="space-y-6 py-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {Object.entries(BRIEFING_TEMPLATES).map(
                                ([key, template]) => (
                                  <button
                                    key={key}
